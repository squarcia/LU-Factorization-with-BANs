...
class Ban{
	...
	//new constructor!!!
	Ban(int p, const T num[SIZE], int check);
	///////////////////////////////////////
	Ban(T n);
    Ban(const Ban& b);
    ...
    void denoise(T tol);
	//added functions
	inline T get_mon(int i) const{return num[i];};
	inline void set_mon(T num[SIZE]){for(int i = 0; i< SIZE; i++) this->num[i] = num[i];};
	//NEW!!!!
	bool comp(Ban b);
};
#pragma pack(pop)
...
//constexpr T _[] = {1.0, 0, 0, 0, 0, 0, 0, 0};
//constexpr T __[] = {0.0, 0, 0, 0, 0, 0, 0, 0};
const Ban ALPHA(1, _);
const Ban ETA(-1, _);
const Ban ZERO(0, __);
const Ban ONE(0, _);
